...
export default function PublicationEdit({ publication, sdgs, experts }) {
    const { data, setData, post, errors, processing } = useForm({
        sdg_id: publication.sdg_id ?? '',
        title: publication.title ?? '',
        year: publication.year ?? '',
        link: publication.link ?? '',
        experts: publication.experts.map(expert => expert.id) ?? [], // Ensure we get the selected experts' IDs
        _method: 'PUT', // Ensure it's an update request
    });
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        post(route('admin.publications.update', publication.id), data); // Update the route for publication
    };
            ...
            <Container>
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Edit the existing publication entry</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <form onSubmit={handleSubmit}>
                            ...
                            <div className="mt-6">
                                ...
                                <Link
                                    href={route('admin.publications.index')} // Adjusted to point to publication index route
...
PublicationEdit.layout = (page) => <AdminLayout children={page} />;
